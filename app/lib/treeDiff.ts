// https://github.com/tylerbwilliams/js-tree-diff/blob/master/src/tree-diff.js
...
export type Tree = InternalNode | LeafNode

type Patch = {
	op: 'DELETE' | 'INSERT' | 'REORDER' | 'UPDATE'
	node: {
		key: number
		path: number[]
	}
...
export const each = (node: Tree, fn: (node: Tree) => void) => {
	fn(node)
	if ('children' in node) {
...
const getInfo = (node: Tree) => ({
	key: node.key,
	path: node.path,
})

// Diff method.
export function diff(tree1: Tree, tree2: Tree): Patch[] {
	...
	const hashes: { [key: number]: { [key: string]: number } } = {}
	...
	const match = (set1: Tree[], set2: Tree[]) => {
